...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function rotateWords() {
    const prevEl = document.querySelector(".hero__word--prev");
    const currentEl = document.querySelector(".hero__word--current");
    const nextEl = document.querySelector(".hero__word--next");
...
document.addEventListener('DOMContentLoaded', () => {
  // Configuración visual del fondo animado
  const config = {
    spacing: 200,
    itemsVisible: 18,
    duration: 6,
  ...
  const logos = [
    ...
    "static/img/linkedin.svg", "static/img/linux.svg", "static/img/notion-svgrepo-com.svg",
    "static/img/photoshop-svgrepo-com.svg", "static/img/python.svg", "static/img/ubuntu-svgrepo-com.svg",
  ]
  ...
  const generateLogos = () => {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        slot.style.left = `${offsetX + c * config.spacing + config.spacing / 2}px`
        slot.style.top  = `${offsetY + r * config.spacing + config.spacing / 2}px`
  ...
  // Renderizar habilidades según categoría
